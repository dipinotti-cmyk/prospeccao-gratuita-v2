...
b) UMA linha dizendo como você chegou até ele, com dado real do briefing
   ...
   fato, cuidado: é PROIBIDO afirmar "você não tem site". Você não sabe disso —
   sabe apenas o que está no perfil do Google dele, e tem gente com site que
   não linkou lá. Diga o que o briefing mostra em "Link no perfil do Google":
   se for Instagram, Linktree ou Doctoralia, cite o que viu ("no seu perfil do
   Google o link vai pro Instagram"); se for "nenhum", diga que não achou link
   de site no perfil. É a mesma informação, só que verificável — e se ele tiver
   site, você não passa por quem não pesquisou. No máximo UM reconhecimento curto, e só se houver nota ou
...
export async function generateLeadMessage({ lead, niche, apiKey }) {
  ...
  // Referencia da 2a mensagem, direto do nicho. Trocar 'modelo' por 'cliente'
  // e mentir pro lead, entao a falta do campo cai sempre no lado seguro.
  const demoUrl = String(niche?.demo_url || '').trim();
  const querDemo = Boolean(demoUrl);
  const ehCaseReal = querDemo && niche?.demo_tipo === 'cliente';
