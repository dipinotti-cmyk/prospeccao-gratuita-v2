...
d) uma frase dizendo que o dele não seria igual — a razão é sempre POSITIVA
   ("a loja dele fica com a cara da marca dele, com os produtos e as fotos
   dele"), NUNCA uma frase sobre o que você não vai fazer ou não vai
   oferecer. É PROIBIDO qualquer frase no molde "eu não faço/crio X antes de
   Y" nesta mensagem — isso é recusa, não venda;
...
- NÃO ofereça montar um rascunho, protótipo, print ou proposta NOVA e exclusiva
  pra esse lead agora — o que existe pra mostrar é só o link da mensagem 2.
  Mas isso é uma instrução de o que NÃO OFERECER, nunca vira frase pro lead
  ler: é PROIBIDO escrever qualquer variação de "eu não crio prévia antes de
  fechar", "não faço protótipo sem compromisso" ou parecido. Frase assim soa
  sovina e contradiz o próprio argumento de venda dele, que É fazer prévia sem
  pedirem. Se o assunto não foi levantado, não precisa dizer o que você não
  vai fazer — simplesmente não ofereça, e ponto.
...
${formatoJson}`;
  ...
  for (const candidato of fila) {
    ...
    if (bruto) {
      ...
      demo = `Ah, e pra você não precisar imaginar: esse aqui é um site que eu fiz, da ${ref.quem}.\n\n${demoUrl}\n\nO seu não seria igual, porque o seu público é outro. ${ref.fechamento || 'Quer que eu te mostre como ficaria com o seu conteúdo?'}`;
    } else if (!ehFallbackHome) {
      demo = `Ah, e pra você não precisar imaginar, olha esse modelo que eu montei pra esse tipo de negócio:\n\n${demoUrl}\n\nÉ um modelo, não é de cliente. No seu entram suas fotos, seus serviços e o contato caindo direto no seu WhatsApp. ${ref.fechamento || 'Me diz o que achou?'}`;
    } else {
      demo = `Ah, e de quebra: aqui você vê outros projetos de loja virtual que eu já entreguei.\n\n${demoUrl}\n\nCada um com a cara da marca do cliente, não é modelo pronto. ${ref.fechamento || 'Quer que eu te mostre como ficaria com a sua marca?'}`;
    }
