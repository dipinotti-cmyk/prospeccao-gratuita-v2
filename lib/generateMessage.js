...
const REFERENCIAS = {
  nutricionista: {
    ...
    quem: 'uma nutricionista em Florianopolis',
    olhar: [
...
export async function generateLeadMessage({ lead, niche, apiKey }) {
  ...
  if (querDemo && ehCaseReal) {
    contextoDemo = `O link da segunda mensagem e um site REAL, de cliente REAL, no ar, feito por voce: ${ref.quem}. Diga isso na primeira pessoa, com naturalidade ("fiz o site de ..."), porque isso e prova entregue e nao maquete. E PROIBIDO chamar de prototipo, modelo ou exemplo.${olharTxt}\nDepois dos pontos, uma frase dizendo que o dele nao seria igual, porque o negocio e outro.${fechamentoTxt}`;
  } else if (querDemo) {
