...
//
// 02/09/2026 (3): GEMINI_API_KEY virou uma chave PAGA (o Diogo trocou e
// redeployou). Free tier ficou pra trás — ver troca de modelo logo abaixo em
// CANDIDATOS e o preço real em lib/pricing.js.
//
...
//
// 02/09/2026 (3): chave virou paga (Diogo pediu "um modelo inteligente que
// não gaste tanto"). gemini-3.5-flash foi pro topo da fila -- já estava na
// lista (validado, funciona nesta conta), e o preço real ($1.50/$9 por
// milhão de tokens, input/output) fica ABAIXO do gemini-3.1-pro ($2/$12)
// com pontuação melhor em benchmark de codificação, segundo o pricing
// publicado em benchlm.ai/google/api-pricing e metacto.com (setembro/2026)
// -- ou seja, mais inteligente que o pro por um preço menor, não é o modelo
// mais barato do catalogo (isso continua sendo os -flash-lite, guardados
// como fallback). Os -flash-lite baixaram de posição mas continuam na fila:
// servem de rede se o flash cair ou a conta ficar sem credito.
const CANDIDATOS = [
...
// Chamada à Gemini com resposta em JSON, percorrendo a fila de CANDIDATOS.
//
// 03/09/2026: extraída daqui de dentro pra ser reusada pela geração da
// MENSAGEM SEGUINTE (lib/generateReply.js, quando o lead responde). A fila de
// modelos, o retry de 429 e o parse tolerante do JSON são os mesmos nos dois
// lugares — duplicar isso significaria um dia consertar o 429 só na metade.
//
// Devolve { parsed, usage, model }. Lança erro quando nenhum modelo respondeu
// ou quando o que veio não era JSON.
export async function chamarIAJson({ system, user, apiKey, temperature = 0.9 }) {
  ...
  for (const candidato of fila) {
    ...
    // 03/09/2026 (2): JSON quebrado deixou de derrubar a geração inteira. O
    // caso real foi resposta CORTADA no meio (finish_reason "length") na
    // mensagem seguinte: o texto vinha bom e morria no meio da frase. Cair
    // pro próximo modelo da fila é o que a fila existe pra fazer; devolver
    // meia mensagem pro Diogo mandar no WhatsApp seria bem pior.
    if (!parsed) {
      tentativas.push(`${candidato.model} -> JSON invalido (${choice?.finish_reason || 'sem finish_reason'}): ${limpo.slice(0, 80)}`);
      continue;
    }

    return { parsed, usage: json.usage || null, model: candidato.model };
  }
...
PASSO 0 — QUALIFICAR ANTES DE ESCREVER UMA PALAVRA. Olhe o nome e a categoria
do negócio no briefing e decida: ele vende PRODUTO FÍSICO PRÓPRIO, que caberia
num catálogo de loja virtual com foto, preço e botão de comprar?
...
Conta como NÃO: serviço puro (conserto, manutenção, assistência técnica,
costureira que só ajusta), feira ou mercado de rua, casa de leilões,
atacadista que só revende marca de terceiro, restaurante, bar, salão, clínica,
escritório, e qualquer coisa sem produto próprio pra vender pela internet.

Se for NÃO, devolva exatamente {"qualificado": false, "motivo": "até 12
palavras dizendo o que o negócio é"} e PARE — não escreva "abertura" nem
"demo_intro". Forçar discurso de loja virtual em quem vende serviço queima o
contato. Na dúvida entre os dois, qualifique como NÃO.

Se for SIM, devolva "qualificado": true e escreva as DUAS mensagens abaixo,
que serão enviadas uma logo depois da outra.
...
c) a DOR, como OPORTUNIDADE que hoje passa por fora, nunca como acusação de
   limitação do negócio dele. Fale do dinheiro que fica fora do alcance dele
   HOJE — quem mora em outra cidade e não consegue comprar sem ir até lá,
   quem pesquisa o produto no Google antes de decidir e não encontra loja
   nenhuma pra fechar ali. É PROIBIDO qualquer frase que presuma que o
   negócio está travado, limitado, fechado, parado ou incompleto: nunca
   escreva "trava", "travado", "travada", "limitado", "limita", "fica preso",
   "não dá conta", "fecha à noite", "fecha no feriado", "fora do horário de
   atendimento" nem qualquer variação que sugira que ele vende pouco ou vende
   mal hoje — você não sabe disso, e presumir é o tipo de frase que gera
   resposta defensiva ("minha loja não fica travada") em vez de resposta de
   venda. A dor certa é sobre ALCANCE (gente de fora que ele ainda não
   alcança), nunca sobre HORÁRIO ou CAPACIDADE do negócio dele. É também
...
- PROIBIDO qualquer palavra ou frase que sugira que o negócio dele está
  travado, limitado, parado, fechado ou incompleto hoje ("trava", "travar",
  "travado", "travada", "limitado", "limita", "fica preso", "não dá conta",
  "fora do horário", "fecha à noite"). Isso vale pra dor, pergunta e qualquer
  outra linha. Motivo real: um lead respondeu na defensiva citando de volta a
  palavra "travadas" depois de receber esse tipo de insinuação (09/09/2026).
...
// 09/09/2026: item 1 trocado. "a loja fisica fecha a noite e no feriado"
// presumia horario de atendimento do lead sem dado nenhum no briefing pra
// sustentar isso -- um lead real (horario 24h/todos os dias no Google)
// respondeu na defensiva, citando de volta a palavra "travadas". O angulo
// certo e sobre ALCANCE (quem mora longe, nunca sobre horario/capacidade).
const ANGULOS = [
  'quem mora em outra cidade nao consegue comprar sem ir ate a loja fisica; uma loja virtual no ar vende pra fora tambem',
...
const OFERTA_LABEL = {
  ...
  // 15/09/2026: os dois modos novos não usam este mapa pra gerar mensagem
  // (generateLeadMessageLojaExistente tem prompt próprio) — o rótulo aqui
  // serve só de referência pra quem olhar o dado no painel/dashboard.
  'diagnostico-nuvemshop': 'diagnóstico gratuito de loja Nuvemshop já existente',
  'migracao-plataforma': 'diagnóstico gratuito e avaliação de migração pra Nuvemshop',
};
...
//
// Retorna { qualificado, motivo, message, demo, demoUrl, subject, usage, model }.
//   qualificado = false quando o lead não vende produto físico próprio
//                 (conserto, feira, leilão, serviço puro). Nesse caso message
//                 e demo voltam '' e motivo diz o que o negócio é — quem
//                 chamou decide o que fazer (regenerate.js marca "descartado").
//   message = mensagem de abertura (mantém o nome antigo, pra não quebrar quem já importa)
//   demo    = segunda mensagem, sempre presente quando qualificado — case/modelo
//             do nicho, ou a home do site (diogopinotti.com.br) quando o nicho
...
export async function generateLeadMessage({ lead, niche, apiKey }) {
  ...
  // 02/09/2026 (3): rótulo corrigido pra bater com o conteúdo real do campo.
  // Era "sobre pedir demonstração grátis" — nome que empurrava o modelo a
  // oferecer prévia, um dos gatilhos do bug corrigido acima (ver PADRAO_RECUSA).
  // O campo pedido_demo continua com esse nome no banco, mas hoje guarda a
  // PROVA que pode ser citada (parceiro certificado, case entregue), não a
  // demo que se oferece.
  ...
  const formatoJson = isEmail
    ? 'Responda em JSON: {"qualificado": true ou false, "motivo": "só quando qualificado for false", "subject": "assunto curto, 5 a 8 palavras, sem clickbait", "abertura": "corpo do e-mail", "demo_intro": "emenda + {LINK} + pontos pra reparar, SEM fechar a mensagem"}'
    : 'Responda em JSON: {"qualificado": true ou false, "motivo": "só quando qualificado for false", "abertura": "primeira mensagem de whatsapp", "demo_intro": "emenda + {LINK} + pontos pra reparar, SEM fechar a mensagem"}';
...
${formatoJson}`;
  ...
  const { parsed, usage, model } = await chamarIAJson({
  ...
  // 02/09/2026 (4): qualificação vem antes de tudo. Lead que não vende
  // produto físico próprio (conserto, feira, leilão) sai daqui sem
  // mensagem nenhuma — quem chamou decide o que fazer (regenerate.js marca
  // como "descartado"). Não gerar mensagem é o resultado CERTO aqui, não erro.
  if (parsed.qualificado === false) {
    return {
      qualificado: false,
      motivo: semTravessao(parsed.motivo || '') || 'não vende produto físico próprio',
      message: '',
      demo: '',
      demoUrl: null,
      subject: null,
  ...
  if (!abertura) {
    throw new Error(`A Gemini (${model}) respondeu sem o campo "abertura".`);
  }
...
// ————— Lead que JÁ TEM loja — modos 'diagnostico-nuvemshop' e
// 'migracao-plataforma' (15/09/2026) —————
//
// Público diferente do resto do arquivo: os 26 nichos acima miram quem NÃO
// vende online ainda. Aqui é o oposto — a Apify só traz o lead pra este modo
// depois que pages/api/apify-webhook.js detectou (lib/detectarPlataforma.js)
// que ele JÁ tem loja rodando, então não existe "vende produto físico?" pra
// qualificar: se chegou aqui, já é loja de e-commerce de verdade.
//
// Só a mensagem 1 (abertura) é gerada. A mensagem 2 — a que oferta o
// diagnóstico/migração com autoridade — só nasce DEPOIS que o lead responder
// (mesmo padrão de generateReply.js), porque o preço/próximo passo depende
// do que ele disser. Escrever essa mensagem 2 hoje, sem uma resposta real
// pra calibrar, seria repetir o erro que generateReply.js já cometeu uma vez
// (3 textos genéricos reprovados antes da estrutura atual).
//
// Aprovado pelo Diogo em 15/09/2026, depois de duas rodadas de revisão: a
// pergunta de fechamento NÃO pode ser múltipla escolha (ele mesmo apontou:
// "eu responderia: nada") — tem que ser uma pergunta aberta que deixa a dor
// real aparecer, igual à regra que MENSAGEM 1 já segue pros 26 nichos.
const REGRAS_UNIVERSAIS_LOJA_EXISTENTE = `PERGUNTA — é o que decide se a mensagem funciona. UMA pergunta, a última
linha, em parágrafo próprio, ABERTA (a pessoa responde com uma frase, não com
sim/não/uma letra de múltipla escolha). É PROIBIDO oferecer opções pra
escolher ("é A, B ou C?", "é venda, tempo ou visibilidade?") — isso vira
questionário, e questionário de estranho ninguém responde. É PROIBIDO
pergunta vazia: "O que você acha?", "Faz sentido pra você?", "Podemos
conversar?", "Posso te mandar mais detalhes?". A pergunta certa deixa a dor
REAL da pessoa aparecer, sem o remetente advinhar qual é.

- Nunca abra criticando a loja ou a plataforma que ela usa hoje.
- Nunca presuma que a pessoa quer trocar de plataforma ou que a loja atual
  tem problema. "Vi que vocês usam X" é observação; "vi que X não é bom" é
  ataque, e ataque ao que a pessoa já escolheu gera defesa, não resposta.
- NÃO abra com "Meu nome é Diogo" nem qualquer auto-apresentação — o nome já
  aparece no perfil do WhatsApp de quem manda, repetir é espaço perdido.
- NÃO lidere com credencial, currículo ou "Especialista Certificado
  Nuvemshop" nesta primeira mensagem — isso só entra DEPOIS que a pessoa
  responder (mensagem 2, escrita por outro processo). Primeiro o negócio
  dela, depois você.
- NÃO ofereça diagnóstico, análise ou qualquer serviço nesta mensagem — só
  descubra a dor. A oferta vem na mensagem 2, depois que ela falar.
- NÃO use travessão. NÃO escreva "espero que esteja bem", "venho por meio
  deste", "sei que seu tempo é valioso" nem qualquer clichê de IA.
- NÃO invente NOME PRÓPRIO (de pessoa, loja, bairro) que não esteja escrito
  no briefing, letra por letra. Sem nome no briefing, escreva sem nome.
- Cite no máximo 1 dado de mercado, e só se reforçar o ponto.
- No máximo um emoji, e só se couber natural.
- Nunca repita a mesma primeira frase de outra mensagem.
- O nome do negócio precisa aparecer.
- No máximo 60 palavras. Parágrafos de uma linha, separados por linha em
  branco — é WhatsApp no celular, não carta.

Responda SEMPRE em JSON válido, sem nenhum texto fora do JSON:
{"qualificado": true, "abertura": "a mensagem pronta"}`;

const SYSTEM_PROMPT_JA_TEM_NUVEMSHOP = `Você escreve a PRIMEIRA mensagem de contato para o dono de uma loja Nuvemshop já
existente, encontrada pelo Google Maps. Ninguém pediu diagnóstico ainda —
esta mensagem só puxa conversa sobre a operação dele, nada mais.

Nesta ordem:
a) saudação de verdade com o PRIMEIRO nome extraído do nome do negócio no
   briefing (se o briefing trouxer nome de pessoa; senão, sem saudação por
   nome, direto pro negócio);
b) UMA linha dizendo que você viu que o negócio já tem loja na Nuvemshop —
   fato verificável, sem elogio inventado;
c) UMA linha dizendo que você trabalha especificamente com configuração e
   otimização de loja Nuvemshop (não currículo, só o que você faz);
d) a pergunta aberta sobre o que mais dá trabalho na operação da loja hoje —
   varie a forma entre leads, sempre aberta, nunca múltipla escolha.

${REGRAS_UNIVERSAIS_LOJA_EXISTENTE}`;

const SYSTEM_PROMPT_OUTRA_PLATAFORMA = `Você escreve a PRIMEIRA mensagem de contato para o dono de uma loja de
e-commerce numa plataforma que NÃO é a Nuvemshop (Shopify, Wix, WooCommerce,
VTEX, Loja Integrada, Tray ou Magento — o briefing diz qual), encontrada pelo
Google Maps. Ninguém pediu migração ainda — esta mensagem só puxa conversa
sobre a experiência dele com a plataforma atual, nada mais.

Nesta ordem:
a) saudação de verdade com o PRIMEIRO nome extraído do nome do negócio no
   briefing (se o briefing trouxer nome de pessoa; senão, sem saudação por
   nome, direto pro negócio);
b) UMA linha dizendo que você viu a loja e em que plataforma ela roda hoje —
   fato verificável, tom neutro, NUNCA insinuando que a plataforma é ruim;
c) UMA linha dizendo que você trabalha com e-commerce (não currículo, não
   "sou especialista", só o que você faz);
d) a pergunta aberta sobre como tem sido a experiência com a plataforma
   atual no dia a dia — varie a forma entre leads, sempre aberta, nunca
   múltipla escolha, nunca perguntando se ela quer trocar.

${REGRAS_UNIVERSAIS_LOJA_EXISTENTE}`;

// lead: { name, category, city, rating, reviews_count, website, site_tipo }
// modo: 'diagnostico-nuvemshop' | 'migracao-plataforma'
// Retorna { qualificado: true, message, usage, model } — sem demo/link,
// mensagem 2 nasce depois, na resposta do lead.
export async function generateLeadMessageLojaExistente({ lead, modo, apiKey }) {
  const system = modo === 'migracao-plataforma' ? SYSTEM_PROMPT_OUTRA_PLATAFORMA : SYSTEM_PROMPT_JA_TEM_NUVEMSHOP;
...
Site: ${lead.website || 'não informado'}`;
  ...
  const { parsed, usage, model } = await chamarIAJson({ system, user: userPrompt, apiKey, temperature: 0.9 });

  const abertura = semTravessao(parsed.abertura || '');
  if (!abertura) {
    throw new Error(`A Gemini (${model}) respondeu sem o campo "abertura".`);
  }

  return { qualificado: true, message: abertura, usage, model };
}
